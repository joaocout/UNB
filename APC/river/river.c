...
void gameover(int x){/*x eh o motivo do fim de jogo*//*1 eh batida*//*0 eh falta de combustivel*//*2 eh atingido por <*/
    if(!fim){
        if(ranked)
            registermatch();
        system(CLEAR);
    ...
    for(linha=1; linha<(altura-1); linha++){
        for(coluna=0; coluna<largura; coluna=coluna+(largura-1)){
            if(tabuleiro[linha][coluna]=='>')/*remove os '>' do fim do mapa*/
                tabuleiro[linha][coluna]=' ';
                /*remove mobs do fim do mapa*/
            if(tabuleiro[linha][coluna]=='X' || tabuleiro[linha][coluna]=='F' || tabuleiro[linha][coluna] == 'O' || tabuleiro[linha][coluna]=='T')
                tabuleiro[linha][coluna]=' ';
            /*destruicao dos < no fim do mapa*/
...
void tiroT(){
	...
	for(linha=1; linha<altura-1; linha++){
		for(coluna=0; coluna<largura; coluna++){
			int r = RAND();
			if(tabuleiro[linha][coluna]=='T' && r<=2){
	...
	for(linha=1; linha<altura-1; linha++){
		for(coluna=0; coluna<largura; coluna++){
			if(tabuleiro[linha][coluna]=='T' && (RAND())<=3){/*chance pequena para evitar mts tiros ao mesmo tempo na tela*/
				int j;
				for(j=0; j<=numeroT; j++){
					if(inimigosT[j].posicao == coluna)
						if(inimigosT[j].municao>0){
							inimigosT[j].municao--;
							tabuleiro[linha][coluna-1]='<';			
						}
				}
    ...
    for(linha=1; linha<(altura-1); linha++){
        for(coluna=1; coluna<largura; coluna++){
            ...
            /*colisao entre + e O*/
             ...
             /*colisao entre '+' e 'T'*/
            else if(tabuleiro[linha][coluna]=='T' && tabuleiro[linha][coluna-1]=='+')
                gameover(1);
            /*colisao entre + e <*/
            else if(tabuleiro[linha][coluna]=='<' && tabuleiro[linha][coluna-2]=='+')
            	gameover(2);
            else if(tabuleiro[linha][coluna]=='<' && tabuleiro[linha][coluna-3]=='+')
            	gameover(2);
            else if(tabuleiro[linha][coluna]=='<' && tabuleiro[linha][coluna-1]=='+')
            	gameover(2);
            ...
            /*movimento normal dos O*/
                ...
                int j;/*att da posicao do o na struct*/
                for(j=0; j<=numeroO; j++){
            ...
            /*movimento normal dos T*/
                ...
                int j;/*atualizacao da posicao do t na struct*/
                for(j=0; j<=numeroT; j++){
                    if(inimigosT[j].posicao==coluna){
                        inimigosT[j].posicao--;
                    }
            ...
            /*moviemtno normal dos <*/
            	...
            	else if(tabuleiro[linha][coluna-2]=='X' || tabuleiro[linha][coluna-2]=='F' || tabuleiro[linha][coluna-2]=='O' || tabuleiro[linha][coluna-2]=='T'){
            		tabuleiro[linha][coluna]=' ';
...
void spawn() {
    ...
    if(x<=probF){
        ...
        if(tabuleiro[r][coluna-1]==' '){
            ...
            inimigosT[numeroT].municao=municaoT;
            numeroT++;
        }
        if(numeroT==200){
            numeroT=0;
...
void atirar(int posicaonave){
    ...
    else if(tabuleiro[posicaonave][1]=='T')/*caso exista um x na frente da nave*/
        tabuleiro[posicaonave][1]=' ';
    else if(tabuleiro[posicaonave][1]=='<')/*caso exista um x na frente da nave*/
        tabuleiro[posicaonave][1]=' ';
    else if(tabuleiro[posicaonave][1]=='O'){/*caso exista um O na frente da nave*/
                int j;
                for(j=0; j<=numeroO; j++){
                    if(inimigosO[j].posicao==1 && inimigosO[j].vida>0)
                        inimigosO[j].vida--;
                    if(inimigosO[j].vida==0 && inimigosO[j].posicao==1){
                        destruirO();
                        tabuleiro[posicaonave][1]=' ';
                    }
                }
    ...
    if((*posicaonave)>1){
        if(c==119 || c==87){/*w pressionado*/ /*movimento para cima*/
            if(tabuleiro[(*posicaonave)-1][0]=='F'){/*encontro vertical entre '+' e 'F'*/
                ...
                gameover(1);
            else if(tabuleiro[(*posicaonave)-1][0]=='T')/*encontro vertical entre '+' e 'T'*/
                gameover(1);
            else if(tabuleiro[(*posicaonave)-1][0]=='<')/*encontro vertical entre '+' e '<'*/
                gameover(2);
            else if(tabuleiro[(*posicaonave)-1][0]=='O'){/*encontro entre + e O*/
                tabuleiro[(*posicaonave)-1][0]='+';
                tabuleiro[(*posicaonave)][0]=' ';
            }
